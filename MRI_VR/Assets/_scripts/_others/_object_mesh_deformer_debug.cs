...
public class _object_mesh_deformer_debug : MonoBehaviour
{
    [Header("Parameters")]
    public float springForce = 20f;
    public float damping = 5f;
    public float f_attenuatedForce_multiplier = 1000;
    ...
    ////    vertexVelocities[i] = velocity;
    ////    displacedVertices[i] += velocity * (Time_deltaTime / uniformScale);*/
    ////    displacedVertices[i] = originalVertices[i] + velocity * springForce;
    ////    vertexVelocities[i] = new Vector3(0, 0, 0);
    ////    last_displacedVertices[i] = displacedVertices[i];
    ////}

    ////public void AddDeformingForce(Vector3 point, float force, Vector3 end_stick)
    ////{
    ////    point = transform.InverseTransformPoint(point);
    ////    for (int i = 0; i < displacedVertices.Length; i++)
    ////    {
    ////        AddForceToVertex(i, point, force, end_stick);
    ////    }
    ////}

    ////void AddForceToVertex(int i, Vector3 point, float force, Vector3 end_stick)
    ////{
    ...
    ////        //float velocity = attenuatedForce * Time_deltaTime;
    ////        //vertexVelocities[i] += pointToVertex.normalized * velocity;
    ////        vertexVelocities[i] = 1 / (Vector3.Distance(point, end_stick)) * new Vector3(1, 1, 1);
    ////    }
    ////}
    ////public float max_distance_deformation = 0.1f;
    ...
    //[HideInInspector]
    public bool b_is_deformable = true;
    private void Update()
    {
        if (b_debug_get_mesh)
        {
