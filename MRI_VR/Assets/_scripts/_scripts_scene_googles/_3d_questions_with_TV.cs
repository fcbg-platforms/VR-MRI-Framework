...
public class _3d_questions_with_TV : MonoBehaviour {

    private GameObject QuestionSceneText;

    [Header("Save / Load Avatar Configuration")]
    private string SaveFilePath = "Results/AnswersWithTV.txt";
	...
	void Update () {
        if (expyVRstarted)
        {
            ...
            if (IsAnswerTrue)
            {
				...
				_script_main_experiment_manager.s_last_input_command = "question_with_TV";
			}
            if (IsAnswerFalse)
            {
                expyVRintAnswer = 0;
    ...
    public void expyVRwriteQuestion()
    {
        ...
        writer.WriteLine(timestamp + " [question_with_TV] [0] - " + expyVRquestion + " - " + saved_left_text + " - " + saved_right_text + " - " + saved_i_TimeNoTextBeforeDisplayQuestion + " - " + saved_i_TimeSelectedSymbolDisplay + " - " + saved_path_of_the_picture);
    ...
    public void expyVRwriteAnswers()
    {
        ...
        string timestamp = DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture);
		writer.WriteLine(timestamp + " [question_with_TV] [1] - " + ((expyVRintAnswer == 0) ? "[left] " + saved_right_text : ((expyVRintAnswer == 1) ? "[right] " + saved_left_text : "[none] " + "No Answer")));
		writer.WriteLine("");
