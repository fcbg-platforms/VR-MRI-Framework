...
public class _3d_questions : MonoBehaviour {

    private GameObject QuestionSceneText;

    [Header("Save / Load Avatar Configuration")]
    private string SaveFilePath = "Results/Answers.txt";
	...
	void Update () {
        if (expyVRstarted)
        {
            ...
            if (IsAnswerTrue)
            {
				...
				_script_main_experiment_manager.s_last_input_answer = "answer_true";
				_script_main_experiment_manager.s_last_input_command = "question";
            ...
            if (IsAnswerFalse)
            {
				...
				_script_main_experiment_manager.s_last_input_answer = "answer_false";
				_script_main_experiment_manager.s_last_input_command = "question";
			}
    ...
    public void ExpyVRStartQuestion(string question, string left_text, string right_text, int i_TimeNoTextBeforeDisplayQuestion, int i_TimeSelectedSymbolDisplay)
    {
    ...
    public void expyVRwriteQuestion()
    {
        ...
        writer.WriteLine(timestamp + " [question] [0] - " + expyVRquestion + " - " + saved_left_text + " - " + saved_right_text + " - " + saved_i_TimeNoTextBeforeDisplayQuestion + " - " + saved_i_TimeSelectedSymbolDisplay);
    ...
    public void expyVRwriteAnswers()
    {
        ...
        string timestamp = DateTime.UtcNow.ToString("yyyy-MM-dd HH:mm:ss.fff", CultureInfo.InvariantCulture);
        writer.WriteLine(timestamp + " [question] [1] - " + ((expyVRintAnswer == 0) ? "[left] " + saved_right_text : ((expyVRintAnswer == 1) ? "[right] " + saved_left_text : "[none] " + "No Answer")));
        writer.WriteLine("");
